...
public class DataHubTopic {
    ...
    private static Logger logger = LoggerFactory.getLogger(DataHubTopic.class);
    ...
    public DataHubTopic(String businessDataitemId,DataHubShardCache dataHubShardCache) throws Exception {
        try {
            this.topicName = SG3761SqlMapping.getInstance().getTopicName(businessDataitemId);
            this.activeShardList = dataHubShardCache.getActiveShard(SG3761SqlMapping.getInstance().getProjectName(businessDataitemId), topicName);
