...
import com.tl.easb.utils.AutoTaskCacheUtils;
 ...
 */
public class AutoTaskManage {
	private static Logger log = LoggerFactory.getLogger(AutoTaskManage.class);

	private static AutoTaskManage autoTaskManager = null;


	private AutoTaskManage() {

	}

	public static synchronized AutoTaskManage getInstance() {
		if (null == autoTaskManager)
			autoTaskManager = new AutoTaskManage();
	...
	public static AutoTaskConfig findTaskConfig(String autotaskId) {
		AutoTaskConfig autoTaskConfig = AutoTaskCacheUtils.findTask(autotaskId);
		if(null != autoTaskConfig){
			return autoTaskConfig;
		}
		synchronized (AutoTaskManage.class) {
			autoTaskConfig = AutoTaskCacheUtils.findTask(autotaskId);
			if(null != autoTaskConfig){
				return autoTaskConfig;
			}
			log.error("Can not find task ["+autotaskId+"] in redis,will be initing!");
			initAutoTaskConfig(autotaskId);
		}
		return AutoTaskCacheUtils.findTask(autotaskId);
	}
	...
	public static void removeTaskConfig(String autotaskId){
		AutoTaskCacheUtils.delTask(autotaskId);
	...
	private static void initAutoTaskConfig(String autotaskId){
		try {
			...
			while(rs.next()){
				...
				if(rs.isLast()){
					AutoTaskCacheUtils.initTask(rs.getString("AUTOTASK_ID"), ac);
				}
			}
	...
	public void dealCollectTask(String taskType, String autoTaskId, String colDataDate) throws SchedulerException {
		/**
		 * 新增和修改时需要删除redis中任务信息，重新进行init
		 */
		if("0".equals(taskType)||"1".equals(taskType)){
			AutoTaskManage.removeTaskConfig(autoTaskId);
		}
		AutoTaskConfig taskConfig = AutoTaskManage.findTaskConfig(autoTaskId);
		if(null == taskConfig){
		...
		if (null != taskConfig.getState() && taskConfig.getState().equals("01")) {
			...
			if ("0".equals(taskType)) {
				...
				AutoTaskCacheUtils.initTask(taskConfig.getAutoTaskId(),taskConfig);
			} else if ("1".equals(taskType)) {
				...
				AutoTaskCacheUtils.delTask(taskConfig.getAutoTaskId());
			} else if ("3".equals(taskType)) {
