...
public class DataHubControl {
    ...
    public PutRecordsResult insertion(String project, List<DataObject> doList, RefreshDataCache refreshprocessor) {
        ...
        for (DataObject dataObject : doList) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("You have a new message write to project " + project + " topic " + topicName + " and shard id " + dataObject.getShardId());
            }
            if (batchTopicFlag != topicName) {
                ...
                if (putRecordsResult.getFailedRecordCount() > 0) {
                    logger.error("Warning!!!!!!Write to datahub " + batchTopicFlag + " failed! one of their shardId is " + putRecordsResult.getFailedRecords().get(0).getShardId());
                }
                batchTopicFlag = topicName;
                recordEntries.clear();
        ...
        try {
            putRecordsResult = datahubClient.putRecords(project, batchTopicFlag, recordEntries);
        } catch (Exception e) {
            StringBuilder sb = new StringBuilder();
            for (RecordEntry re : recordEntries
            ) {
                JsonNode jsonNode = re.toJsonNode();
                sb.append(jsonNode.toString());
            }
            logger.error("写入datahub异常，异常topic:" + batchTopicFlag + "  recordSize:" + recordEntries.size() + " recordDetails:" + sb.toString(), e);
            throw new RuntimeException("DATAHUB_EXCEPTION", e);
        }
        putRecordsResult = refresh(project, batchTopicFlag, refreshKeyArrList, refreshprocessor, putRecordsResult);
        if (putRecordsResult.getFailedRecordCount() > 0) {
            logger.error("Warning!!!!!!Write to datahub " + batchTopicFlag + " failed! one of their shardId is " + putRecordsResult.getFailedRecords().get(0).getShardId() + "; ErrorMessage: " + putRecordsResult.getFailedRecordError().get(0).getMessage());
        }
        recordEntries.clear();
        refreshKeyArrList.clear();
    ...
    private PutRecordsResult refresh(String project, String topicName, List<Object[]> refreshKeyArrList, RefreshDataCache refreshprocessor, PutRecordsResult putRecordsResult) {
        if (putRecordsResult.getFailedRecordCount() == 0) {
            ...
            logger.error("Warning!!!!!!Write to datahub " + topicName + " failed! one of their shardId is " + putRecordsResult.getFailedRecords().get(0).getShardId() + ",to update DataHub-shard-cache.");
